...
$GET = explode('/',$url);
$dId = isset($GET[2]) && is_numeric($GET[2]) ? $GET[2]:0;
$aId = isset($GET[4]) && is_numeric($GET[4]) ? $GET[4]:0;
?>
						...
						<div class="box-group" id="accordion">
						    <div class="box-primary">
							  	...
							  	<div class="" style="border-bottom:1px solid #ccc;">
									<?php
									   echo $this->Form->Open($this->Url->getBaseUrl().($dId?'link/update/'.$dId:'link/add/'),'frmItem',array('enctype'=>'multipart/form-data'));
									   echo $this->Form->DropDown('drpDevAppId',$dId!=0?$dId:'','','Developer Account',array(),array(),$developer_ac);
									   if($dId){
									   		echo $this->Form->DropDown('drpAppId',$aId!=0?$aId:'','','App Name',array(),array(),$app_info);
									  	}else{
									   		echo $this->Form->DropDown('drpAppId','','','App Name','','','');		  
									  	}
						    ...
						    <div class="box-primary">
							   	<div class="box-header" style="border-bottom:1px solid #ccc;">
									<h3 class="box-title">Link App</h3>
										...
										<button id="setPriority" class="btn btn-primary" style="color:#fff">Set Priority</button>
								  	</h3>
							   	</div>
...
<script type="text/javascript">
	jQuery(document).ready(function(e) {
		...
		$("#setPriority").click(function(){
			var appid=[];
			var index=[];
			$('#LinkApp tbody>tr').each(function(){
				appid.push($(this).attr('id'));
				index.push($("#"+$(this).attr('id')).index());
			});
			
			$.ajax({
					url: "/appAdmin/index.php/api/setpriority",
					type: "post",
					data: {
						aId: <?php echo $aId;?>,
		...
		$("#drpDevAppId").change(function(){
			 window.location.href='/appAdmin/index.php/admin/link/dev/'+$(this).val();
		});
		$("#drpAppId").change(function(){
			 window.location.href='/appAdmin/index.php/admin/link/dev/'+<?php echo $dId;?>+'/app/'+$(this).val();
		});
		$("#LinkTable td:last-child a:first-child .iCheck-helper").click(function(){ 
			var id=$(this).attr('id');
			window.location.href='/appAdmin/index.php/admin/link/dev/'+<?php echo $dId;?>+'/app/'+<?php echo $aId;?>+'/link/'+id;
		});
		$("#LinkApp td:last-child a:first-child .iCheck-helper").click(function(){ 
			var id=$(this).attr('id');
			window.location.href='/appAdmin/index.php/admin/link/dev/'+<?php echo $dId;?>+'/app/'+<?php echo $aId;?>+'/unlink/'+id;
		});
		$("#LinkApp td:last-child a:nth-child(2) .iCheck-helper").click(function(){ 
			var id=$(this).attr('id');
			window.location.href='/appAdmin/index.php/admin/link/dev/'+<?php echo $dId;?>+'/app/'+<?php echo $aId;?>+'/banner/'+id;
		});
